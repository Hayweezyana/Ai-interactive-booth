...
const keyPath = env_1.env.GOOGLE_APPLICATION_CREDENTIALS || process.env.GOOGLE_APPLICATION_CREDENTIALS;
...
const createClient = () => {
    if (!keyPath) {
        console.error("DEBUG: env object says:", env_1.env.GOOGLE_APPLICATION_CREDENTIALS);
    ...
    return new aiplatform_1.PredictionServiceClient({
        // apiEndpoint: `${env.GOOGLE_CLOUD_LOCATION}-aiplatform.googleapis.com`,
        apiEndpoint: `${location}-aiplatform.googleapis.com`,
        keyFilename: keyPath,
    });
};
async function generateVertexImage(prompt, base64Images) {
    const client = createClient(); // Initialize here
    const projectId = process.env.GOOGLE_CLOUD_PROJECT || env_1.env.GOOGLE_CLOUD_PROJECT;
    const location = 'us-central1';
